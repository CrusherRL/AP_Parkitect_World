...
class ParkitectWorld(World):
    ...
    def set_rules(self) -> None:
        ...
        def set_parkitect_rule(rule_type, selected_item, location_number):
            entrance = self.multiworld.get_region(get_previous_region_from_parkitect_location(location_number), self.player).entrances[0]

            #logger.info(f"-> set_parkitect_rule rule_type: {rule_type}")
            #logger.info(f"-> set_parkitect_rule selected_item: {selected_item}")
            #logger.info(f"-> set_parkitect_rule location_number: {location_number}")
            #logger.info(f"-> set_parkitect_rule previous_region: {get_previous_region_from_parkitect_location(location_number)}")
            #logger.info(f"Entrance {entrance.name} existing rule: {entrance.access_rule}")
        ...
        for number, item in enumerate(self.item_table):
            ...
            if self.random.random() < difficulty_modifier:
                ...
                #thing = possible_prereqs[0]
                #thing = possible_prereqs[len(possible_prereqs) - 1]
                #logger.info(f"          Prereq: {thing}")

                if thing in item_info["Rides"]:
                    set_parkitect_rule("ride", thing, number)

                elif thing in item_info["Shops"]:
                    set_parkitect_rule("shop", thing, number)
